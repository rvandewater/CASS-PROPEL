...
from src.utils.plot import save_editable_plot
...
def shap_summary(X, model_label, out_dir, shap_tuples, editable=True):
    """
    ...
    pickle.dump(appended_shap, open(f'{out_dir}/{model_label}_SHAP_values.pkl', 'wb'))
    ...
    plt.tight_layout()
    output_format = "pdf"
    plt.savefig(f'{out_dir}/{model_label}_SHAP_summary.{output_format}'.replace(' ', '_'),
                format=output_format)
    if editable:
        save_editable_plot(f'{out_dir}/{model_label}_SHAP_summary')
    plt.close()
    shap.summary_plot(appended_shap, features=test_df, plot_type='bar', max_display=num_features, show=False)
    plt.tight_layout()
    plt.savefig(f'{out_dir}/{model_label}_SHAP_summary_bars.{output_format}'.replace(' ', '_'),
                format=output_format)
    if editable:
        save_editable_plot(f'{out_dir}/{model_label}_SHAP_summary_bars')
    plt.close()
    # shap.plots.bar(mean_shap_values, max_display=num_features, show=False)
    # plt.tight_layout()
    # plt.savefig(f'{out_dir}/{model_label}_SHAP_bars.{output_format}'.replace(' ', '_'), format=output_format)
